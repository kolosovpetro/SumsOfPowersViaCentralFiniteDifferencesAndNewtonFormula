...
\begin{corollary}[Centered ordinary sums of powers]
  For integers $t, \; m\geq 0, \; n \geq 1$
  \begin{align*}
    \KnuthRFoldSum{1}{n}{m}
...
\begin{align*}
  ...
  % \Bigg [
  %   k
  %   \left( \sum_{j=1}^n \binom{j-t+\frac{k}{2}-1}{k} \right)
  %   - \left [-t-\frac{k}{2} \right ]
  %   \left( \sum_{j=1}^n \binom{j-t+\frac{k}{2}-1}{k-1} \right)
  %   - t \sum_{j=1}^{n} \binom{j-t+\frac{k}{2}-1}{k-1}
  % \Bigg ]
\end{align*}
% To maintain parity of the arguments $m,k$ of central differences
% $\delta^{k} t^m$, we set $m \rightarrow 2m$ and let degree $k$ to runs
% over even values, so that $\delta^{k} t^m \neq 0$.
% Therefore,

% \begin{proposition}[Even powers in central binomial form]
%   \label{prop:even-powers-in-central-binomial-form}
%   For integers $n,t$ and $m \geq 0$
%   \begin{align*}
%     n^{2m}
%     = t^{2m} + \sum_{k=1}^{m} \frac{n-t}{2k}
%     \binom{n-t+k-1}{2k-1} \delta^{2k} t^{2m}
%   \end{align*}
% \end{proposition}
